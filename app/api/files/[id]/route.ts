...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  try {
    await requireUser();
    const file = await db.file.findUnique({ where: { id: params.id }, include: { tags: { include: { tag: true } }, folder: true } });
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (folderId && !(await db.folder.findUnique({ where: { id: folderId } }))) {
    ...
    const updated = await db.$transaction(async (tx) => {
      ...
      const next = await tx.file.update({
        ...
        data: {
          ...
          folderId,
          visibility,
          publishedAt: visibility === 'PUBLIC' ? file.publishedAt || new Date() : null,
