...
export default async function CollectionDetailPage({ params }: { params: { slug: string } }) {
  const collection = await db.collection.findFirst({
    ...
    include: {
      files: { include: { file: { include: { tags: { include: { tag: true } } } } }, orderBy: { order: 'asc' } },
    },
  });
  ...
  return (
    <main className="shell" style={{ paddingTop: 32, paddingBottom: 80 }}>
      ...
      <section style={{ paddingTop: 56, paddingBottom: 30 }}>
        <p className="accent">COLLECTION</p>
        <h1 style={{ fontSize: 'clamp(2.4rem, 6vw, 5rem)', margin: 0 }}>{collection.name}</h1>
        <p className="muted" style={{ maxWidth: 680, lineHeight: 1.8, marginTop: 16 }}>
          {collection.description || 'A collection from the archive.'}
        </p>
      </section>
            ...
            <Link key={file.id} href={`/file/${file.id}`} className="glass file-card" style={{ padding: 10 }}>
              <div className="preview">
                {file.mimeType.startsWith('image/') ? (
                  ...
                  <span className="accent" style={{ fontSize: 24, letterSpacing: '0.14em' }}>
